use core::{
    barline::BarlineType,
    clef::ClefSignature,
    complex::{create_complexes_for_one_voice, create_complexes_for_two_voices},
    context::CoreContext,
    direction::DirectionUD,
    hpart::{HPartAttributes, HPartItem, HPartItems, HPartMusicType, HPartType, VoiceType2},
    key::KeySignature,
...
#[allow(unused_mut)]
impl Parse2Utils {
    pub fn create_columns_and_rows_of_parts(_cx: &CoreContext, bpvmap: &mut [SysItemTypeId], parts_count: usize) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for item in bpvmap.iter() {
            let mut column_parts: Vec<HPartType> = vec![];

            let mut column_duration: usize = 0;
            for part_idx in 0..parts_count {
                let hpart: HPartType = match item {
                    SysItemTypeId::Clefs(segments) => {
                        ...
                        HPartType::Clef(clef_signature)
                    }
                    SysItemTypeId::Barlines(segments) => {
                        let _item = &segments[part_idx];
                        HPartType::Barline(BarlineType::Single)
                    }
                            ...
                            1 => {
                                ...
                                let complexes = match &voicetype {
                                    VoiceType2::NoteIds { note_ids, duration, stemitem_ids: _ } => {
                                        ...
                                        create_complexes_for_one_voice(_cx, &note_ids, *duration, true, part_idx, Some(column_position))
                                    }
                                    _ => Vec::new(),
                                };
                            ...
                            2 => {
                                ...
                                let complexes = match (&upper, &lower) {
                                    (
                                        ...
                                        create_complexes_for_two_voices(_cx, &upper_ids, &lower_ids, *upper_dur, *lower_dur, Some(column_position))
                                    }

                                    (_, VoiceType2::NoteIds { note_ids, duration, stemitem_ids: _ }) => {
                                        column_duration = column_duration.max(*duration);
                                        create_complexes_for_one_voice(_cx, &note_ids, *duration, false, part_idx, Some(column_position))
                                    }
                ...
                column_parts.push(hpart);
            }

            let ids = column_parts
                .into_iter()
                .map(|hpart| {
                    ...
                    let item: HPartItem = HPartItem {
                        ...
                        part_idx: 0, // This will be set later
                    };
            ...
            let column: HPartItems = HPartItems { id, hpart_ids: ids };
        ...
        for part_idx in 0..parts_count {
            ...
            for column in _cx.columns.borrow().iter() {
                let item = column.hpart_ids.get(part_idx);
            ...
            let row: HPartItems = HPartItems { id, hpart_ids: ids };
